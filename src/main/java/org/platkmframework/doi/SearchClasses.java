...
import org.platkmframework.annotation.PropertyFileContent;
...
public class SearchClasses implements IoDProcess {
     ...
     * Atributo logger
     */
    private static Logger logger = LoggerFactory.getLogger(SearchClasses.class);

    /**
     * Atributo C_SERVICE_ANNNOTATION_ACTIVE
     */
    public static final String C_SERVICE_ANNNOTATION_ACTIVE = "service.annotation.active";

    /**
     * Constructor SearchClasses
     */
     ...
     * @param packagesPrefix packagesPrefix
     * @param objectReferece objectReferece
     * @return Map
     * @throws IoDCException IoDCException
     */
    @Override
    public Map<Object, List<Method>> process(String packageNames, String[] packagesPrefix, ObjectReferece objectReferece) throws IoDCException {
        ...
        try {
            if (StringUtils.isNotEmpty(packageNames)) {
                //packageNames.split(",") solo si se buscan los paquetes por l informacion en web.xml
                String[] arrPackages = packageNames.split(";");
                if (arrPackages != null) {
                    for (int i = 0; i < arrPackages.length; i++) {
                        process(arrPackages[i], packagesPrefix, objectReferece, mField, mInterface, listPendingClass, methods);
                    }
                }
            }
     ...
     */
    private void _processPendingClasses(ObjectReferece objectReferece, Map<Field, List<Object>> mField, Map<String, Object> mInterface, List<Constructor<?>> listPendingClass, boolean firstCheck) throws IoDCException, InvocationException, IllegalArgumentException, IllegalAccessException {
        logger.info("PENDING...");
        Class<?>[] parameters;
        ...
        for (Constructor<?> constructor : listPendingClass) {
            ...
            if (parameters != null) {
                ...
                if (listParam.size() == parameters.length) {
                    ...
                    logger.trace("PENDING... " + obj.getClass().getName() + " - " + firstCheck);
     ...
     * @param objectReferece objectReferece
     * @param mField mField
     * @param mInterface mInterface
     * @param listPendingClass listPendingClass
     * @param methods methods
     * @throws IoDCException IoDCException
     * @throws InvocationException InvocationException
     * @throws URISyntaxException URISyntaxException
     */
    private void process(String packageName, String[] packagesPrefix, ObjectReferece objectReferece, Map<Field, List<Object>> mField, Map<String, Object> mInterface, List<Constructor<?>> listPendingClass, Map<Object, List<Method>> methods) throws IoDCException, InvocationException, URISyntaxException {
                        ...
                        if (!entry.isDirectory() && entry.getName().endsWith(".class")) {
                            ...
                            if (containsPrefix(packagesPrefix, className)) {
                                try {
                                    ...
                                    logger.trace(className + " - Exception NOT FOUND");
                                }
                            }
                        }
                    }
     ...
     */
    protected void _process(String packageName, String[] packagesPrefix, File folder, ObjectReferece objectReferece, Map<Field, List<Object>> mField, Map<String, Object> mInterface, List<Constructor<?>> listPendingClass, Map<Object, List<Method>> methods) throws InvocationException, IoDCException, IllegalArgumentException, IllegalAccessException {
        try {
            ...
            if (files != null)
                for (int i = 0; i < files.length; i++) {
                    ...
                    if (file.exists())
                        if (file.isDirectory())
                            _process(packageName + "." + file.getName(), packagesPrefix, file, objectReferece, mField, mInterface, listPendingClass, methods);
     ...
     * containsPrefix
     * @param packagesPrefix packagesPrefix
     * @param classNamePackage classNamePackage
     * @return boolean
     */
     ...
     * @throws IllegalAccessException IllegalAccessException
     */
    private void _classProcess(Class<?> class1, ObjectReferece objectReferece, Map<Field, List<Object>> mField, Map<String, Object> mInterface, List<Constructor<?>> listPendingClass, Map<Object, List<Method>> methods) throws InvocationException, IoDCException, IllegalArgumentException, IllegalAccessException {
        if (class1.isAnnotationPresent(Factory.class) || class1.isAnnotationPresent(Component.class) || isServicesAndActiveChecked(class1, objectReferece) || class1.isAnnotationPresent(Api.class) || class1.isAnnotationPresent(Controller.class) || class1.isAnnotationPresent(Repository.class) || class1.isAnnotationPresent(RMIServer.class) || class1.isAnnotationPresent(SecurityCofing.class) || class1.isAnnotationPresent(DatabaseConfig.class) || class1.isAnnotationPresent(ApplicationConfig.class) || class1.isAnnotationPresent(HttpRest.class)) {
            ...
            if (!_hasCosntructorParam(class1, listPendingClass)) {
                if (class1.isAnnotationPresent(Api.class)) {
                    ...
                    processApiKey(objectReferece, ob, mField);
                } else {
                    Object ob;
                    if (class1.isAnnotationPresent(HttpRest.class)) {
                        ob = Proxy.newProxyInstance(this.getClass().getClassLoader(), new Class<?>[] { class1 }, new InvocationHandler() {

                            @Override
                            public Object invoke(Object proxy, Method method, Object[] args) throws ProxyProcesorException {
            ...
            } else if (class1.isAnnotationPresent(TruslyException.class)) {
                objectReferece.getExceptions().add(class1.getName());
            } else if (class1.isAnnotationPresent(Functional.class)) {
                objectReferece.getFunctionals().add(ReflectionUtil.createInstance(class1));
            } else {
                processCustomClass(objectReferece, class1);
            }
        }
    }
    ...
    private void generalProcess(Object ob, Map<Field, List<Object>> mField, ObjectReferece objectReferece, Map<String, Object> mInterface) throws IllegalArgumentException, IllegalAccessException, IoDCException, InvocationException {
        ...
        _processPropertyInfo(ob, mField, objectReferece);
        _processPropertyFile(ob, mField);
        _referencesByInterface(ob, mInterface);
        logger.trace(ob.getClass().getName());
    }

    /**
     * processApiKey
     * @param objectReferece objectReferece
     * @param controllerObj controllerObj
     * @param mField mField
     * @throws IllegalArgumentException IllegalArgumentException
     * @throws IllegalAccessException IllegalAccessException
     * @throws IoDCException IoDCException
     */
    private void processApiKey(ObjectReferece objectReferece, Object controllerObj, Map<Field, List<Object>> mField) throws IllegalArgumentException, IllegalAccessException, IoDCException {
        _checkAutoWired(controllerObj, mField, objectReferece);
        Method[] methods = controllerObj.getClass().getMethods();
        if (methods != null && methods.length > 0) {
            Method method;
            ClassMethod classMethod;
            Api api = controllerObj.getClass().getAnnotation(Api.class);
            //String key = api.path();
            String apiNameHttpMehtod;
            String pathVariableAPIKey;
            for (int i = 0; i < methods.length; i++) {
                method = methods[i];
                if (method.isAnnotationPresent(ClassMethod.class)) {
                    classMethod = method.getAnnotation(ClassMethod.class);
                    if (StringUtils.isNotBlank(classMethod.method().name())) {
                        apiNameHttpMehtod = api.path() + classMethod.name();
                    } else {
                        apiNameHttpMehtod = api.path();
                    }
                    if (!apiNameHttpMehtod.contains("{")) {
                        objectReferece.addApiInfo(apiNameHttpMehtod + "-" + classMethod.method().name().toLowerCase(), controllerObj, method.getName());
                    } else {
                        pathVariableAPIKey = classMethod.method().name().toLowerCase() + "-" + apiNameHttpMehtod.split("/").length;
                        objectReferece.addApiInfo(pathVariableAPIKey, apiNameHttpMehtod, controllerObj, method.getName());
                    }
                }
            }
        }
    ...
    private boolean isServicesAndActiveChecked(Class<?> class1, ObjectReferece objectReferece) {
        ...
        if (service.key().isEmpty() || service.value().isEmpty())
            return true;
    ...
    private void _referencesByInterface(Object ob, Map<String, Object> mInterface) {
        ...
        if (interfaces != null)
            for (int i = 0; i < interfaces.length; i++) mInterface.put(interfaces[i].getName(), ob);
    }
    ...
    private void _checkAutoWired(Object ob, Map<Field, List<Object>> aux, ObjectReferece objectReferece) throws IoDCException, IllegalArgumentException, IllegalAccessException {
        ...
        for (int i = 0; i < methods.size(); i++) {
            ...
            if (m.isAnnotationPresent(TimerFixeDelayScheduler.class) || m.isAnnotationPresent(TimerFixeRateScheduler.class) || m.isAnnotationPresent(TimerScheduler.class) || m.isAnnotationPresent(SystemColumnValue.class)) {
    ...
    /**
     * _processPropertyFile
     * @param obj obj
     * @param mField mField
     * @throws InvocationException InvocationException
     */
    private void _processPropertyFile(Object obj, Map<Field, List<Object>> mField) throws InvocationException {
        List<Field> fields = ReflectionUtil.getAllFieldHeritage(obj.getClass());
        PropertyFileContent propertyFileInfo;
        boolean accessValue;
        Field f;
        for (int i = 0; i < fields.size(); i++) {
            f = fields.get(i);
            if (f.isAnnotationPresent(PropertyFileContent.class)) {
                propertyFileInfo = f.getAnnotation(PropertyFileContent.class);
                accessValue = f.canAccess(obj);
                f.setAccessible(true);
                try {
                    f.set(obj, getPropertyFile(f, propertyFileInfo.path()));
    ...
    private Object getPropertyFileInfoValue(Field field, String propertyKey, ObjectReferece objectReferece) throws InvocationException {
        if (propertyKey.indexOf("$") >= 0)
            return Util.checkingArgs(propertyKey, objectReferece.getProp());
     ...
     * getPropertyFile
     * @param field field
     * @param propertyFilePath propertyFilePath
     * @return Properties
     * @throws InvocationException InvocationException
     * @throws IOException IOException
     */
    private Properties getPropertyFile(Field field, String propertyFilePath) throws InvocationException, IOException {
     ...
     * @throws InvocationException InvocationException
     */
    private void _processAutowired(ObjectReferece objectReferece, Map<Field, List<Object>> mField, Map<String, Object> mInterface) throws IoDCException, InvocationException {
        try {
            Field f = null;
